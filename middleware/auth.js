...
export const verifyToken = (req, res, next) => {
  ...
  console.log("Header yang masuk:", req.headers.authorization); // TAMBAHKAN INI
  ...
  if (authHeader && authHeader.startsWith("Bearer ")) {
    ...
    jwt.verify(token, process.env.JWT_SECRET, (err, decoded) => {
      if (err) {
        ...
        console.log("Nama Error:", err.name); // misal: JsonWebTokenError
        console.log("Pesan Error:", err.message); // misal: invalid signature atau jwt malformed
